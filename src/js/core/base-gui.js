/*
 * PCPaint X - https://www.parcoil.com/products/pcp
 * author: Parcoil Inc
 */
...
class Base_gui_class {
	...
	set_events() {
		...
		document.getElementById('canvas_PCPaint X').addEventListener('contextmenu', function (e) {
			e.preventDefault();
		}, false);
	}
	...
	check_canvas_offset() {
		...
		var canvas_el = document.getElementById('canvas_PCPaint X').getBoundingClientRect();
		this.canvas_offset.x = canvas_el.left - bodyRect.left;
		this.canvas_offset.y = canvas_el.top - bodyRect.top;
	}

	prepare_canvas() {
		var canvas = document.getElementById('canvas_PCPaint X');
		...
		this.render_canvas_background('canvas_PCPaint X');
